...
def find_max_batch_size(local_rank, world_size, shared_batch_size, start_batch_size=2, max_batch_size=1024):
    ...
    if local_rank == 0:  # rank 0 프로세스만 공유 변수를 업데이트
        shared_batch_size.value = success_batch_size-1
    
    pbar.close()
    dist.destroy_process_group()


def train_ddp(local_rank, world_size, batch_size, epochs=2):
    # DDP 초기화
    dist.init_process_group(backend="nccl", init_method="env://", rank=local_rank, world_size=world_size)
    torch.cuda.set_device(local_rank)
    device = torch.device(f"cuda:{local_rank}")
...
if __name__ == "__main__":
    ...
    # 수정된 find_max_batch_size 함수에 맞게 인자 전달
    torch.multiprocessing.spawn(
        find_max_batch_size, 
        args=(world_size, shared_batch_size), 
        nprocs=world_size
    )
    ...
    torch.multiprocessing.spawn(
        train_ddp, 
        args=(world_size, optimal_batch_size), 
        nprocs=world_size
    )