...
def find_max_batch_size(local_rank, world_size, shared_batch_size, start_batch_size=2, max_batch_size=1024):
    ...
    # 모든 GPU에서 찾은 최소 배치 사이즈로 동기화
    local_batch_size = torch.tensor([success_batch_size], device=f'cuda:{local_rank}')
    global_batch_size_tensor = [torch.zeros_like(local_batch_size) for _ in range(world_size)]
    dist.all_gather(global_batch_size_tensor, local_batch_size)
    min_batch_size = min([tensor.item() for tensor in global_batch_size_tensor])
    
    if local_rank == 0:  # rank 0 프로세스만 공유 변수를 업데이트
        shared_batch_size.value = min_batch_size
...
def train_fsdp(local_rank, world_size, batch_size, epochs=2):
    dist.init_process_group(backend="nccl", init_method="env://", rank=local_rank, world_size=world_size)
    torch.cuda.set_device(local_rank)
    device = torch.device(f"cuda:{local_rank}")
...
if __name__ == "__main__":
    ...
    shared_batch_size = mp.Value('i', 0)

    torch.multiprocessing.spawn(find_max_batch_size, args=(world_size, shared_batch_size), nprocs=world_size)
    optimal_batch_size = shared_batch_size.value
    print(f"Optimal batch size: {optimal_batch_size}")
    
    torch.multiprocessing.spawn(train_fsdp, args=(world_size, optimal_batch_size), nprocs=world_size)
